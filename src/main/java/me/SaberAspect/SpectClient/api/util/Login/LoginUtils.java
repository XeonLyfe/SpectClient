...
public class LoginUtils {
	public static String loginAlt(String email, String password) {
        ...
        try {
            ...
            try {
				...
				f.set(Wrapper.getMinecraft(), new Session(authentication.getSelectedProfile().getName(), authentication.getSelectedProfile().getId().toString(), authentication.getAuthenticatedToken(), "mojang"));
				displayText = "Logged [License]: " + Wrapper.getMinecraft().getSession().getUsername();
			} catch (Exception e) {
    ...
    public static void changeCrackedName(String name) {
        try {
        	Field f = Minecraft.class.getDeclaredField(Mapping.session);
        	f.setAccessible(true);
			f.set(Wrapper.getMinecraft(), new Session(name, "", "", "mojang"));
		} catch (Exception e) {
			e.printStackTrace();
		}
    }
